...
class _AdminDashboardState extends State<AdminDashboard> {
  ...
  Future<void> _fetchCounts() async {
    ...
    try {
      ...
      final internSnap = await FirebaseFirestore.instance
          .collection('users')
          .where('role', isEqualTo: 'intern')
          .get();

      final taskSnap = await FirebaseFirestore.instance.collection('tasks').get();
      ...
      for (var doc in taskSnap.docs) {
        ...
        else if (status.contains('progress')) progress++;
        else if (status.contains('complete')) complete++;
      }
  ...
  Widget build(BuildContext context) {
    ...
    return CommonScaffold(
      title: role == "super admin" ? "Super Admin Dashboard" : "Admin Dashboard",
      role: role,
      body: isLoading
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text("$count", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: KDRTColors.darkBlue)),
              ],
            ),
          ),
        ),
...
Widget _taskCard({
 ...
 Widget _statusChip(String label, int count, Color color) {
    return Chip(
      label: Text("$label: $count", style: const TextStyle(color: Colors.white, fontSize: 12)),
